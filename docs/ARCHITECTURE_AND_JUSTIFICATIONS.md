...
1. **OS-Level Architecture Divergence**: RHEL 9 is required to run the monolithic libvirt daemon (`libvirtd`), whereas RHEL 10 mandates modular driver daemons (`virtqemud`, `virtnetworkd`, `virtstoraged`, etc.).
...
### Step 1: `tasks/preflight.yml` (OS Check & Kernel Stack)

- **What it does**:
  1. Checks that the host is RedHat-family 9 or 10.
...
- **What it does**:
  - **RHEL 9**: enables and starts `libvirtd.service` (Monolithic model, required). No sockets are managed and nothing is masked; the other libvirt daemons stay as the OS ships them.
  - **RHEL 10**: Enables and starts the sockets of all drivers (each daemon starts on demand) and `virtqemud.service`, and masks the legacy `libvirtd` units.
- **Justification**:
  - RHEL 10 completely removes `libvirtd`. Trying to start `libvirtd` on RHEL 10 results in fatal systemd unit-not-found errors.
  - RHEL 9 keeps `libvirtd`, so the role only has to enable and start it. Keeping RHEL 9 to the one service is the simplest way to meet the monolithic requirement.
...
[SCREENSHOT: systemctl status output showing libvirtd active on RHEL 9, vs modular virtqemud active (and libvirtd masked) on RHEL 10]
...
- **What it does**:
  1. Creates `/var/lib/libvirt/images` with directory permissions `0711`, ownership `root:root` and the SELinux label `virt_image_t`.
  2. Records the label in the SELinux policy with `community.general.sefcontext`, so it survives a relabel.
  3. Defines the storage pool with `community.libvirt.virt_pool` (a one-line directory-pool XML), then activates it, then sets autostart, as three separate tasks (`virt_pool` ignores `autostart` when it is combined with `state`; see `docs/LESSONS_LEARNED_AND_FIXES.md` item #4).
- **Justification**:
  ...
  - **Dynamic Storage Path Customization**: Because `/var/lib/libvirt/images` is on the root partition (`/`), administrators frequently redirect VM storage to dedicated RAID/NVMe arrays (e.g. `/data/vms`). Setting `rhel_kvm_storage_pools` lets users choose pools and paths without touching code, while the role dynamically creates the target folder, registers the SELinux context, and configures Libvirt seamlessly.
  ...
  - The `community.libvirt` tasks use the module's default connection, `qemu:///system`, on both OSes.
...
### 2. Global Libvirt System URI Configuration (later removed)
...
- **Implementation**: `/etc/profile.d/libvirt.sh` exporting `LIBVIRT_DEFAULT_URI="qemu:///system"` was deployed, then removed from the role to keep it simple. Use `virsh -c qemu:///system` or `sudo virsh`.
...
- **Retained Action**: the CPU virtualization flag check (`vmx`/`svm`) was kept for a time, then removed: the hypervisors run on a known nested-virtualization setup, so the check never reported anything.
